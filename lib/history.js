...
module.exports = class History {
  ...
  async notifyUsers () {
    ...
    this.getForks().then(res => {
      ...
      const users = this.getUsers(forks)
      return this.createIssue(users)
    })
  }
  ...
  async createIssue (users) {
    ...
    const issueComment = this.config['issueComment']

    if (perform) {
      this.logger.info('Creating an issue on %s/%s', owner, repo)
      if (issueTitle && issueComment) {
        const params = {owner, repo, title: issueTitle, body: issueComment + mention, labels: [historyLabel]}
